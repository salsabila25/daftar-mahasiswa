...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Daftar Mahasiswa - Menggunakan API</h1>
      ...
      <div class="modal-content">
        ...
        <form id="formEdit">
          ...
          <div class="form-group">
              <label for="program" class="form-label">Status Mahasiswa Saat Ini</label>
              <div class="form-check">
                  <input class="form-check-input" type="radio" name="edit_status" id="statusAktif" value="1">
                  <label class="form-check-label" for="statusAktif">
                    Aktif
                  </label>
                </div>
                <div class="form-check-inline">
                  <input class="form-check-input" type="radio" name="edit_status" id="statusLulus" value="0">
                  <label class="form-check-label" for="statusLulus">
                    Lulus
                  </label>
                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.lihatMahasiswa', function(e){
            ...
            $.ajax({
            type: "GET",
            url: "http://localhost:8000/mahasiswa/"+nimMahasiswa,
            typedata: "json",
            success: function(response) {
                var mahasiswa = response[0];
                // console.log(mahasiswa);
                if (response.status == 404) {
                //alert("Data mahasiswa tidak ditemukan");
                $('.isi-biodata').append('Data mahasiswa tidak ditemukan');
                ...
                <tr>\
                    ...
                    <td>'+ (mahasiswa.status == 1 ? "Aktif" : "Lulus") +'</td>\
                    </tr>');
                }
            },
            error: function(error) {
      ...
      $(document).on('click', '.editMahasiswa', function(e){
        ...
        $.ajax({
            ...
            if (response.status == 404) {
                ...
                $('#edit_nim_mahasiswa').val(mahasiswa.nim);
                $('#edit_nim').val(mahasiswa.nim);
                $('#edit_nama').val(mahasiswa.nama);
                $('#edit_tanggal_lahir').val(mahasiswa.tanggal_lahir);
                $('#edit_jurusan').val(mahasiswa.jurusan);
                $('#edit_program_studi').val(mahasiswa.program_studi);
      ...
      console.log($('input[name="edit_status"]:checked').val());
